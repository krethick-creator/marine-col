...
export default function TopBar() {
  const { unreadAlertCount, clearAlertBadge, offlineMode, lastSyncTime, deferredPrompt, setDeferredPrompt, isInstallable, setIsInstallable, isAppInstalled, user, setLanguage } = useAppStore()
  ...
  const appLanguage = user?.language || 'en'
  const currentLang = LANGUAGE_OPTIONS.find((l) => l.code === appLanguage) ?? LANGUAGE_OPTIONS[0]
    ...
    <header className="top-bar">
      ...
      <div className="top-bar-pill" style={{ flexDirection: 'column', alignItems: 'flex-start', padding: '4px 10px', gap: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
          <div className="live-dot" style={{ background: offlineMode ? 'var(--status-caution)' : 'var(--status-go)' }} />
          {offlineMode ? t('topbar.offline').toUpperCase() : t('topbar.liveData').toUpperCase()}
        </div>
        {offlineMode && (
      ...
      <button
        ...
        id="alerts-bell-btn"
        aria-label={t('topbar.viewAlerts')}
        style={{ position: 'relative', cursor: 'pointer', padding: '6px 10px' }}
      >
        <Bell size={14} />
        {unreadAlertCount > 0 && (
          <span style={{ position: 'absolute', top: 2, right: 2, width: 8, height: 8, borderRadius: '50%', background: 'var(--status-nogo)', boxShadow: '0 0 0 2px rgba(239,68,68,0.3)' }} />
        )}
      </button>
      ...
      <div style={{ position: 'relative' }} ref={langRef}>
        ...
        >
          <Globe size={14} />
          <span style={{ fontSize: 12, fontWeight: 600, letterSpacing: '0.03em' }}>{currentLang.code.toUpperCase()}</span>
          <ChevronDown size={11} style={{ transition: 'transform 0.2s', transform: langOpen ? 'rotate(180deg)' : 'rotate(0deg)' }} />
        </button>

        {langOpen && (
          <div style={{ position: 'absolute', top: 'calc(100% + 8px)', right: 0, minWidth: 200, background: 'var(--surface-2, #1a2235)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 10, boxShadow: '0 8px 32px rgba(0,0,0,0.5)', zIndex: 1000, overflow: 'hidden', animation: 'fadeSlideIn 0.15s ease' }}>
            ...
            {LANGUAGE_OPTIONS.map((opt) => {
              const isActive = opt.code === appLanguage
              return (
                <button
                  key={opt.code}
                  id={`lang-option-${opt.code}`}
                  onClick={() => handleSelectLanguage(opt.code)}
                  style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%', padding: '8px 14px', background: isActive ? 'rgba(59,130,246,0.15)' : 'transparent', border: 'none', cursor: 'pointer', color: isActive ? 'var(--accent-blue, #3b82f6)' : 'var(--text-primary, #e2e8f0)', fontSize: 13, textAlign: 'left', transition: 'background 0.12s', gap: 10 }}
                  onMouseEnter={(e) => {
