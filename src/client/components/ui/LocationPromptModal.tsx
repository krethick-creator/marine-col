    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        ...
        try {
          ...
          const locName = data.address?.city || data.address?.town || data.address?.county || 'Current Location'
          onSelectLocation({ lat: latitude, lon: longitude }, locName)
        } catch (err) {
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
            {QUICK_LOCATIONS.map((loc, i) => (
              <button
                ...
                onClick={() => onSelectLocation({ lat: loc.lat, lon: loc.lon }, loc.name)}
              >
                {loc.name.split(',')[0]}
              </button>
            ))}
